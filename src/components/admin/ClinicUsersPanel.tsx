...
import { adminApi, type ClinicUser, ApiError } from "@/lib/admin/api";
...
export function ClinicUsersPanel({ clinicId }: { clinicId: number }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        phone: form.phone || null, role: form.role, status: form.status,
      };
      if (form.password) {
  ...
  return (
    <section aria-label="Clinic users" style={{ marginTop: 24 }}>
      <div className="adm-page-hdr">
        <div>
          <h2 className="adm-page-title">Clinic Users</h2>
          <div className="adm-page-sub">Manage login accounts, user details and passwords for this clinic.</div>
          {!loading && !loadError && <div className="adm-page-sub">{total} users in this clinic</div>}
        </div>
      ...
      {loading ? (
        ...
        <Empty message="Could not load clinic users." action={<Btn variant="ghost" onClick={load}>Retry</Btn>} />
      ) : users.length === 0 ? (
                  ...
                  <tr key={u.id}>
                    ...
                    <td>
                      <div className="adm-actions">
                        <Btn variant="ghost" size="sm" onClick={() => openEdit(u)}>Edit / Password</Btn>
      ...
      <Modal
        open={Boolean(modal)}
        onClose={() => { if (!saving) { setModal(null); setForm(EMPTY_USER_FORM); } }}
      ...
      >
        ...
        <div className="adm-form-grid">
          <Field label="Full Name" required error={formErrors.full_name}>
            <input className={`adm-input${formErrors.full_name ? " err" : ""}`} aria-label="Full Name" value={form.full_name} onChange={(e) => setF("full_name", e.target.value)} />
          </Field>
          ...
          <Field label={editUser ? "Password (leave blank to keep)" : "Password"} error={formErrors.password}>
            <input className={`adm-input${formErrors.password ? " err" : ""}`} type="password" aria-label="Password" autoComplete="new-password" value={form.password} onChange={(e) => setF("password", e.target.value)} />
          </Field>
          <Field label="Confirm Password" error={formErrors.confirm_password}>
            <input className={`adm-input${formErrors.confirm_password ? " err" : ""}`} type="password" aria-label="Confirm Password" autoComplete="new-password" value={form.confirm_password} onChange={(e) => setF("confirm_password", e.target.value)} />
          </Field>
        </div>
        <Field label="Status">
